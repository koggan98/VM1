...
class HandPositionPublisher(Node):
    ...
    def transform_camera_to_world(self, camera_coords):
        try:
            ...
            self.get_logger().warn(f"Keine Transformationen verfügbar. Verwende Fallback-Transformation. Fehler: {e}")

            # Annahme, dass die Kamera -0.5 m in y-Richtung von der Roboterbasis verschoben ist
            T_camera_to_robot_base = np.array([
                [1,  0,  0,  0],
                [0,  0,  1, -0.5],
                [0, -1,  0,  0],
                [0,  0,  0,  1]
                ...
                for hand in hand_landmarks:
                    ...
                    if z is not None:
                        ...
                        if self.gesture_tracker.detect_double_open_close(hand, self.mp_tracker.mp_hands):
                            ...
                            hand_position.orientation.x = 1.0
                            hand_position.orientation.y = 0.0
                            hand_position.orientation.z = 0.0
                            hand_position.orientation.w = 0.0
